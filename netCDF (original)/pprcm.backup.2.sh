...
#=================================================== 
obs=( "$OBS_DATA/precip.ymon.mean.nc" "$OBS_DATA/air.ymon.mean.nc" )
...
#=================================================== 
if [ -z "$1" ]; then
	echo "Useage: pprcm + RegCM output directory [ + prefix_of_output_file]"
	exit 1
else
	if [ $# -eq 1 ]; then
		cd $1
...
do
	...
	do
		if [ $nof -lt 1 ];then
			...
			echo "================" $file
			echo "cdo -b 64 selvar,${varlist[$i]} $file ${outname[$i]}.$file.temp"
			cdo -b 64 selvar,${varlist[$i]} $file ${outname[$i]}.$file.temp 2>&-
		fi
	done
	echo "cdo -b 64 mergetime ${outname[$i]}*$name*.temp ${outname[$i]}.all.nc"
	...
	cdo -b 64 ymonmean ${outname[$i]}.all.nc ${outname[$i]}.all.nc.temp
	echo "cdo -b 64 ymonmean ${outname[$i]}.all.nc ${outname[$i]}.all.nc.temp"
...
do
	...
	if [ ${sta_vars[$j]} = "pr" ]; then # change units: kgm-2.s-1 to mm/day
		...
		ncatted -O -a units,pr,o,c,mm/day ${sta_vars[$j]}.out.temp.nc
		#cdo -s pardes ${sta_vars[$j]}.out.temp.nc
		#cdo -s infov ${sta_vars[$j]}.out.temp.nc 
	fi
#=================================================== 
	...
	cdo -s infov bias.${sta_vars[$j]}.temp.nc

#=================================================== clean up
	rm ${sta_vars[$j]}.*.temp.nc ${obs[$j]}.*.temp.nc
