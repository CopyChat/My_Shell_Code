...
shift $(($OPTIND - 1))
#=================================================== 
Start=${1:-1979};End=${2:-1998} 		#  default
...
{
	address=http://clima-dods.ictp.it/data/d8/cordex/EIN15/
	EIN15=/worktmp2/RegCM_DATA/EIN15
...
Pfifo="/Users/tang/.tmp/$$.fifo" # make a pipe line  named as PID
mkfifo $Pfifo   
	...
	do
		...
		do
			...
			if [ "$TEST" = "1" ]; then color 7 1 "-t for TEST!";
				( CMD $j; sleep 1; echo >&6 ) &  # give back the ticket for next job
			else
				( Download $year $param $Time; sleep 3; echo >&6 ) &  # give back the ticket for next job
			fi
				((j++)) 		  # for test
