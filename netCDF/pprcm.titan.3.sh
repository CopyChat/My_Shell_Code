...
EIN_DATA=/worktmp2/RegCM_DATA/5yrMean/
############################################################
#
...
#=================================================== set the data to be used
	if [ $monthly -eq 1 ]; then
		if [ $cross -eq 1 ]; then 
			rm $tarfile 2>&-
			cdo2 merge $EIN_DATA/precip.ymonmean.ein15.nc $EIN_DATA/air.ymonmean.ein15.nc $tarfile
		fi
		if [ $ein -eq 1 ]; then
		...
		if [ $cross -eq 1 ]; then 
			rm $tarfile 2>&-
			cdo2 merge $EIN_DATA/precip.5ymean.ein15.nc $EIN_DATA/air.5ymean.ein15.nc $tarfile
		fi
		if [ $ein -eq 1 ]; then
...
#=================================================== mask the land
		if [ "$mask" = "land" ]; then
			if [ $cross = "1" ] ; then 
				echo "I cannot do this at this moment."
				exit 1
			fi
			mask.sh -l $tarfile
			mv $mask.$tarfile $tarfile
		elif [ "$mask" = "ocean" ]; then
			if [ $cross = "1" ] ; then 
...
#=================================================== mask the land
		if [ "$mask" != "0" ]; then
			...
			overwrite.sh "cdo2 -b 64 mul $tarfile $MASK/$mask.nc" $tarfile
			overwrite.sh "cdo2 -b 64 mul ${obs[$j]}.obs.temp.nc $MASK/$mask.nc " ${obs[$j]}.obs.temp.nc
		fi
#=================================================== select variables
		echo "cdo2 -s selvar,${sta_vars[$j]} $tarfile ${sta_vars[$j]}.out.temp.nc"
		cdo2 -s selvar,${sta_vars[$j]} $tarfile ${sta_vars[$j]}.out.temp.nc
#=================================================== monthly mean
	if [ $monthly -eq 0 ]; then
		cdo2 -s -b 64 timmean ${sta_vars[$j]}.out.temp.nc ${sta_vars[$j]}.out.1.temp.nc
...
# if tarfile is Regcm output precip, then kg m-2.s-1 -->  mm/day
		if [ ${sta_vars[$j]} = "pr" ] && [ $cross -eq 0 ]; then 
			cdo2 -b 64 mulc,86400 ${sta_vars[$j]}.out.temp.nc ${sta_vars[$j]}.out.temp.temp.nc 2>&-
...
#==================================================== calculate
		...
		#echo "cdo2 -s -b 64 fldcor ${obs[$j]}.obs.temp.nc ${sta_vars[$j]}.out.temp.nc ${sta_vars[$j]}.cor.nc"
		cdo2 -s -b 64 fldcor ${obs[$j]}.obs.temp.nc ${sta_vars[$j]}.out.temp.nc ${sta_vars[$j]}.cor.nc 2>&-
		cdo2 -s infov ${sta_vars[$j]}.cor.nc  
		echo "______________________________________________________________________________________________"
		echo "mean" ${sta_vars[$j]} ":"
		cdo2 -s -b 64 fldmean ${sta_vars[$j]}.out.temp.nc ${sta_vars[$j]}.fldmean.nc 2>&-
		cdo2 -s infov ${sta_vars[$j]}.fldmean.nc 
		echo "______________________________________________________________________________________________"
		echo "obs mean" ${sta_vars[$j]}":" 
		cdo2 -s -b 64 fldmean ${obs[$j]}.obs.temp.nc ${obs[$j]}.fldmean.nc 2>&-
		cdo2 -s infov ${obs[$j]}.fldmean.nc 
		echo "______________________________________________________________________________________________"
		echo "bias mean" ${sta_vars[$j]}[ model-obs]":"
		cdo2 -b 64 sub ${sta_vars[$j]}.fldmean.nc ${obs[$j]}.fldmean.nc mean.bias.${sta_vars[$j]}.temp.nc 2>&-
		cdo2 -s infov mean.bias.${sta_vars[$j]}.temp.nc 
		echo "______________________________________________________________________________________________"
		echo "RMSE ${sta_vars[$j]} [ model vs obs ]:"
		cdo2 -b 64 -s sub ${sta_vars[$j]}.out.temp.nc ${obs[$j]}.obs.temp.nc bias.${sta_vars[$j]}.temp.nc 2>&-
		cdo2 -b 64 -s sqr bias.${sta_vars[$j]}.temp.nc sqr.${sta_vars[$j]}.temp.nc 2>&-
		cdo2 -b 64 -s fldmean sqr.${sta_vars[$j]}.temp.nc fld.sqr.${sta_vars[$j]}.temp.nc 2>&-
		cdo2 -b 64 -s sqrt fld.sqr.${sta_vars[$j]}.temp.nc rmse.${sta_vars[$j]}.temp.nc 2>&-
		cdo2 -s infov rmse.${sta_vars[$j]}.temp.nc
		echo "=============================================================================================="
		echo "=============================================================================================="
